...
// ─── Module-level cache: load once per serverless instance, not per request ───
...
async function getVectorStore(): Promise<MemoryVectorStore> {
  ...
  const storePath = path.join(process.cwd(), "public", "vector_store.json");
  if (!fs.existsSync(storePath)) {
    throw new Error(
      "vector_store.json not found in public/. Run: npx tsx scripts/ingest-memory.ts"
    );
  }
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!apiKey) {
      return NextResponse.json(
        { error: "Server configuration error: No OPENROUTER_API_KEY or OPENAI_API_KEY found in environment variables." },
        { status: 500 }
      );
    }
...
{context}`,
    ...
    // If the primary model fails (e.g., 503 capacity limit), the loop tries other free
    // high-quality models from OpenRouter until one succeeds.
    const MODELS = [
    ...
    for (const modelName of MODELS) {
      try {
        ...
        answer = await chainWithHistory.invoke(
          { question },
